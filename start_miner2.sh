...
if [[ $# -lt 1 || -z "${1:-}" ]]; then
  ...
  echo "MODEL (opcjonalnie): gen7heur9"
  exit 1
fi
...
case "$MODEL" in
  ...
  "gen7heur9" )
    ;;
  * )
    echo "ERROR: Niepoprawny MODEL='$MODEL'. Dozwolone: gen7heur9"
    exit 1
    ;;
esac
